...
def create_teacher(user=None, school=None):
    if not user:
        user = create_user()
    if not school:
...
def create_class_room(name=None, school=None, teacher = None):
    '''
...
def create_class_teacher(classroom=None, teacher=None):
...
def create_term(name=None, school=None, start_date=None, end_date=None ):
    '''
    the name of the class defaults to primary 1
    '''
    if not school:
